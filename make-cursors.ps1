...
function Invoke-ChromaKeyWhite {
    ...
    # 1) white-key the opaque pixels (skip already-erased flame pixels, keep them transparent)
    for ($i = 0; $i -lt $len; $i += 4) {
        if ($px[$i + 3] -eq 0) { continue }
        $mn = [Math]::Min($px[$i + 2], [Math]::Min($px[$i + 1], $px[$i]))
        if ($mn -ge 235) { $px[$i + 3] = 0; $px[$i] = 0; $px[$i + 1] = 0; $px[$i + 2] = 0 }
        elseif ($mn -ge 180) { $px[$i + 3] = [byte]((235 - $mn) * 4) }
        else { $px[$i + 3] = 255 }
    }
    # 2) clear stale RGB under any remaining transparent pixels (e.g. flame halo remnants
    #    whose RGB survived an alpha-only erase) so resampling cannot bleed them back
    for ($i = 0; $i -lt $len; $i += 4) {
        if ($px[$i + 3] -eq 0) { $px[$i] = 0; $px[$i + 1] = 0; $px[$i + 2] = 0 }
    }
...
function Invoke-RemoveFlame {
    ...
    # clear stale RGB under erased alpha so later resampling cannot bleed flame color back in
    for ($y = 0; $y -lt $rows; $y++) {
        for ($x = 0; $x -lt $w; $x++) {
            $i = ($y * $s) + ($x * 4)
            if ($px[$i + 3] -eq 0) { $px[$i] = 0; $px[$i + 1] = 0; $px[$i + 2] = 0 }
